...
#保存模型附带数据
rm -rf ${localPath}xgbRelatedRecData
mkdir ${localPath}xgbRelatedRecData 
...
$hadoop fs -get ${hdfs}relatedRecMovie/act ${localPath}xgbRelatedRecData/
...
$hadoop fs -rm -r $output
$hadoop fs -get ${hdfs}relatedRecMovie/direct ${localPath}xgbRelatedRecData/
#加t2 标签 作用同t1一样
...
$hadoop fs -get  ${hdfs}countryArea ${localPath}xgbRelatedRecData/
...
rm ${localPath}xgbRelatedRecData/movieTags
$hadoop fs -get ${hdfs}relatedRecMovie/movieTags ${localPath}xgbRelatedRecData/
$hadoop fs -rm -r $output
...
mv ${localPath}relatedFeatsIdx.txt ${localPath}xgbRelatedRecData/
############
...
$hadoop fs -rm -r $output
$hadoop fs -get $hdfs$movieItemFets ${localPath}xgbRelatedRecData/ 
#export $directStartId
export mvTagsStartId 
export joinStartId
export movieItemFets
export fileidx
sh /home/tvapk/run_sh/cuiliqing/relatedRec/xgbRelatedRecAllShow/ext3.sh
