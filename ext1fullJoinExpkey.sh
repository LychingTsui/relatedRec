...
$hadoop fs -put ${localPath}expFeatKeys ${hdfs}relatedRecMovie/ 
...
$hadoop fs -rm -r $output
$hadoop jar ${jarPath}${jarName} com.qiguo.tv.movie.relatedRec.ExtMovieItemFeaturesAreaExpFeatsKey  $input $output ${hdfs}relatedRecMovie $directStartId $areaStartId  $mvTagsStartId $mvTagsIdxEnd ######???????????
...
export fileidx
sh /home/tvapk/run_sh/cuiliqing/relatedRec/ext2fullJoinExpkey.sh
