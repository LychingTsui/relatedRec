...
$hadoop jar $jarPath$jarName com.qiguo.tv.movie.relatedRec.ConvertRelatedItem2FeaturesJoinTagsLevel  ${hdfs}relatedInput $output $hdfs$movieItemFeatPath  $tagsIdStart $joinTagsStart
$hadoop fs -cp $output/part-r-00000 ${hdfs}relatedDataSet
$hadoop fs -rm -r $output 
rm ${dataSavePath}relatedDataSet 
$hadoop fs -get ${hdfs}relatedDataSet $dataSavePath
$hadoop fs -rm -r ${hdfs}relatedDataSet

totPos=`cat ${dataSavePath}relatedDataSet |awk '$1=="1.0"'| wc -l`
totNeg=`cat ${dataSavePath}relatedDataSet |awk '$1=="0.0"'| wc -l` 
echo "totPos: $totPos"
echo "totNeg: $totNeg"
echo "-----split dataset-----"
...
cat ${dataSavePath}relatedDataSet | awk '$1=="0.0"' | head -$totTrainNeg  > ${dataSavePath}relatedTrain.txt
cat ${dataSavePath}relatedDataSet | awk '$1=="1.0"' | head -$totTrainPos >> ${dataSavePath}relatedTrain.txt
cat ${dataSavePath}relatedDataSet | awk '$1=="0.0"' | tail -$totTestNeg  > ${dataSavePath}relatedTest.txt
cat ${dataSavePath}relatedDataSet | awk '$1=="1.0"' | tail -$totTestPos  >> ${dataSavePath}relatedTest.txt 
echo "-------------------数据完毕-------------------"
